  ...
  def get_product_link(html, selector)
    base_url = "https://fora.kz"
    base_url + html.css(selector).attribute("href").text.strip
  end

  def extract_card_details(card)
    unformatted_price = strip_text(card, "div.item-price p.price")
    {
      title: strip_text(card, "div.item-info a"),
      url: get_product_link(card, "div.item-info a"),
      price: extract_price(unformatted_price),
      code: strip_text(card, "div.item-info ul li:first-child")
    }
  end
  ...
  def make_slug(slug_str, page = 1)
    base_url = "https://fora.kz/catalog/"
    base_url + slug_str + "?page=" + page.to_s
  end

  # check if a card is an injected banner
  def is_banner?(card)
    card_classes = card.attribute("class").text.strip.split

    # return true if there are more than one class detected
    # return  false otherwise
    card_classes.count > 1
  end
    ...
    data.each do |category, sub_categories|
      ...
      sub_categories.each do |sub_category, slugs|
        ...
        slugs.each do |slug|
          ...
          extracted_last_page_num = extract_last_page_num(html.css("div ul.pagination a"))
          # set last page number according to parsed site
          last_page_num = extracted_last_page_num.nil? ? 1 : extracted_last_page_num

          # get number of products per page, ie: 18
          items_per_page = html.css("div.catalog-list-item").count

          if items_per_page < 18 || last_page_num == 1
            # get cards from page and save it as an array
            cards = html.css("div.catalog-list-item")
