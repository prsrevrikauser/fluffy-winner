  ...
  def get_product_link(html, selector)
    base_url = "https://alser.kz"
    base_url + html.css(selector).attribute("href").text.strip
  end

  # get product code from tooltip
  def get_product_code(html, selector)
    html.css(selector).attribute("data-title").text.strip
  end

  def extract_card_details(card)
    unformatted_price = strip_text(card, "div.product-item__info_price div.price")
    {
      title: strip_text(card, "a.product-item__info_title"),
      url: get_product_link(card, "a.product-item__info_title"),
      price: extract_price(unformatted_price),
      code: get_product_code(card, "li.product-item__tooltip")
    }
  end

  def get_alser_json
    file = File::read("./all_cats.json")
    JSON::parse(file)["alser"]
  end

  def make_slug(slug_str, page = 1)
    base_url = "https://alser.kz/c/"
    ...
    data.each do |category, sub_categories|
      ...
      sub_categories.each do |sub_category, slugs|
        ...
        slugs.each do |slug|
          ...
          extracted_last_page_num = extract_last_page_num(html.css("div.pagination a"))
          # set last page number according to parsed site
          last_page_num = extracted_last_page_num.nil? ? 1 : extracted_last_page_num

          # get number of products per page, ie: 16
          items_per_page = html.css("div.product-item").count

          if items_per_page < 16 || last_page_num == 1
            # get cards from page and save it as an array
            cards = html.css("div.product-item")
