  ...
  def get_product_link(html, selector)
    base_url = "https://site.evrika.com"
    base_url + html.css(selector).attribute("href").text.strip
  end

  def extract_card_details(card)
    unformatted_price = strip_text(card, "li.prices-cart__item span.price")
    {
      title: strip_text(card, "p.bx_catalog_item_title a"),
      url: get_product_link(card, "p.bx_catalog_item_title a.link"),
    ...
    data.each do |category, sub_categories|
      ...
      sub_categories.each do |sub_category, slugs|
        ...
        slugs.each do |slug|
          ...
          # get all pagination pages' links
          pagination_links = html.css("ul.pagination-list a")
          # get last page number from parsed html page
          extracted_last_page_num = extract_last_page_num(pagination_links)
          # set last page number according to parsed site
          last_page_num = extracted_last_page_num.nil? ? 1 : extracted_last_page_num

          # get number of products per page, ie: 12
          items_per_page = html.css("div.list-row-products__item").count

          if items_per_page < 12 || last_page_num == 1
            # get cards from page and save it as an array
            cards = html.css("div.list-row-products__item")
