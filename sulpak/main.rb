  ...
  def extract_card_details(card)
    unformatted_price = strip_text(card, "div.price span")
    {
      title: strip_text(card, "h3.title"),
      url: get_product_link(card, "div.product-container-right-side a"),
      price: extract_price(unformatted_price),
      code: strip_text(card, "div.product-container-right-side span.code")
    }
  end

  def get_sulpak_json
    file = File::read("./mbt_categories.json")
    JSON::parse(file)["sulpak"]
  end

  def make_slug(slug_str, page = 1)
    base_url = "https://www.sulpak.kz/f/"
    ...
    data.each do |category, sub_categories|
      ...
      sub_categories.each do |sub_category, slugs|
        ...
        slugs.each do |slug|
          ...
          # get number of products per page, ie: 31
          items_per_page = html.css("div.goods-tiles").count

          if items_per_page < 31 || last_page_num == 1
            # get cards from page and save it as an array
            cards = html.css("div.goods-tiles")
